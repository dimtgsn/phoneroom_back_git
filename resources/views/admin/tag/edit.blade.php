...
@section('content')
    ...
    <h1 class="pt-2 mb-3">Изменить тег:</h1>
    <hr>
    <section class="h-100">
                ...
                <div class="card-wrapper col-8">
                    <div class="">
                        <div class="card card-danger">
                            ...
                            <div class="card-body">
                                <form action="{{ route('admin.tags.update', $tag->slug) }}" method="post" class="pt-3 my-login-validation" enctype="multipart/form-data">
                                    @csrf
                                    @method('patch')
                                    <div class="form-group mb-4">
                                        <label for="name" class="form-label">Название</label>
                                        <input value="{{ $tag->name }}" placeholder="{{ $tag->name }}" name="name" type="text"
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <div class="input-group">
                                            <div class="custom-file">
                                                <label for="image" class="custom-file-label">{{ $tag->image }}</label>
                                                <input value="{{ $tag->image }}" type="file" name="image" class="custom-file-input" id="image">
                                    ...
                                    <div class="form-group" data-select2-id="80">
                                        <label for="products" class="form-label">Товары тега</label>
                                                ...
                                                @foreach($products as $i => $product)
                                                    @if(empty($tag->products[0]) !== true)
                                                        @foreach($tag->products as $tag_product )
                                                            <option {{ ($tag_product->name === $product->name) ? 'selected':''}} value="{{ $product->id }}" data-select2-id="{{$i + 82}}">{{ $product->name }}</option>
                                                        @endforeach
                                                    @else
