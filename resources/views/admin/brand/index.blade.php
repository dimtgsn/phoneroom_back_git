...
@section('content')
    ...
    <div>
        ...
        <div class="row w-100 pt-2 justify-content-between">
            <div class="col">
                <a class="btn d-inline-block btn-primary" href="{{ route('admin.brands.create') }}">Добавить бренд</a>
            </div>
        </div>
        <hr>
        <div class="col-12">
            <table id="example2" class="table table-responsive-lg table-hover dataTable dtr-inline" aria-describedby="example2_info">
                ...
                @foreach($brands as $brand)
                    <tr>
                        ...
                        <th>{{ $brand->name }}</th>
                        <th>
                            <a href="{{ route('admin.brands.show', $brand->slug) }}"><i class="fa fa-solid fa-eye"></i></a>
                            <a class="pl-md-5 pr-md-5 pl-3 pr-3" href="{{ route('admin.brands.edit', $brand->slug) }}"><i class="fa fa-solid fa-pen"></i></a>
                            <form class="m-0 p-0 d-inline-block" action="{{ route('admin.brands.destroy', $brand->slug) }}" method="post">
                                @csrf
                                @method('delete')
                        ...
                        <th>{{ $brand->updated_at ?? 'Изменений нет' }}</th>
                    </tr>
                @endforeach
                </tbody>
                <tfoot>
