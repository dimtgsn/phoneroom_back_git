...
@section('content')
                ...
                <div class="card-wrapper col-8">
                    <div class="">
                        <div class="card card-danger">
                            <div class="card-body">
                                <form action="{{ route('admin.promotion_images.update') }}" method="post" class="pt-3 my-login-validation" enctype="multipart/form-data">
                                    @csrf
                                    @method('patch')
                                        ...
                                        @foreach($images as $img)
                                            <div id="edit-wrapp">
                                                <div class="image-main d-flex flex-column input-group mt-4">
                                                    ...
                                                    <img src="{{ asset($img->path) }}" class="col-7 p-0 edit_image_promotion" alt="Картинка {{ $img->position }}">
                                                    <div class="btn_edit_image_promotion col-7 mt-2 btn btn-danger">Изменить изображение</div>
                                                </div>
    ...
    <script>
        let btn_edit_image_promotion = document.querySelectorAll('.btn_edit_image_promotion');

        const editImagePromotion = ({target}) => {
            let editWrappPromotion = target.parentElement.parentElement;
            let editImagePromotion = editWrappPromotion.querySelector('.edit_image_promotion');
            let cs_file_promotion = editWrappPromotion.querySelector('.custom-file');
            console.log(editWrappPromotion);
            console.log(target);
            if(cs_file_promotion.classList.contains('d-none')){
                cs_file_promotion.classList.remove('d-none');
                editImagePromotion.classList.add('d-none');
                target.innerHTML = 'Отменить изменения';
            }
            else{
                cs_file_promotion.classList.add('d-none');
                editImagePromotion.classList.remove('d-none');
                target.innerHTML = 'Изменить изображение';
            }
        };

        for (let i = 0; i < btn_edit_image_promotion.length; i++) {
            btn_edit_image_promotion[i].addEventListener('click', editImagePromotion, false)
        }
    </script>
@endsection
